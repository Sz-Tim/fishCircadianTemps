...
```{r}
data.df <- readxl::read_xlsx("data/ZF_Data_matrix_stats_RawData2_Gonzalo_LV_120522.xlsx",
                             1, col_types=c(rep("numeric", 6), "skip", "skip")) %>%
  mutate(ln_FishCount=log(FishCount+1),
         cumZT=ZT + (Days-1)*24,
         cos_ZT=cos(2*pi*ZT/24),
         sin_ZT=sin(2*pi*ZT/24),
         Group=factor(Group, labels=c("acclim", "grad", "cte")),
         Chamber=factor(Chamber, ordered=T),
         Tank=case_when(Tank <=3 ~ Tank,
                        between(Tank, 4, 6) ~ Tank - 3,
                        between(Tank, 7, 9) ~ Tank - 6),
         Tank=factor(Tank))
...
```{r}
ggplot(data.df, aes(ZT, ln_FishCount, colour=Group, group=paste(Group, Tank))) + 
  geom_line() + facet_grid(Days~Chamber) +
  scale_colour_viridis_d() + theme_classic()


ggplot(data.df, aes(ZT, fill=is.na(FishCount))) + geom_bar() +
  facet_grid(Chamber~Group)
# More NAs in 0-3h acclim
ggplot(data.df, aes(ZT, fill=is.na(FishCount))) + geom_bar(position="fill") +
  facet_grid(Chamber~Group)

ggplot(data.df, aes(Chamber, ln_FishCount, fill=Group)) + 
  geom_boxplot() + facet_wrap(~Tank)
...
wtChamber.df %>%
  ...
  geom_line(data=wtChamber.sum, aes(y=mn)) +
  geom_ribbon(data=wtChamber.sum, aes(ymin=mn-2*se, ymax=mn+2*se), alpha=0.5, colour=NA) +
  facet_wrap(~Group) +
  ylim(1, 5) +
  labs(x="ZT", y="Preferred chamber")
...
data.df %>% 
  ...
  summarise(mnFish=mean(ln_FishCount, na.rm=T)) %>%
  ggplot(aes(ZT, Chamber, colour=exp(mnFish+1), fill=exp(mnFish+1))) + 
  geom_tile() + 
  facet_wrap(~Group) +
...
RS.out <- brm(ln_FishCount ~ cos_ZT + sin_ZT +
                    cos_ZT:Chamber:Group + 
                    sin_ZT:Chamber:Group + 
                    (1+cos_ZT+sin_ZT|Tank),
               prior=set_prior("normal(0,2)"), 
               control=list(adapt_delta=0.95, max_treedepth=15),
               data=data.noNA, cores=4, refresh=500,
               save_model="models/harmonic_randInt.stan",
               file="models/harmonic_randSlope")
pp_check(RS.out)
summary(RS.out) 
bayes_R2(RS.out) 
```
...
```{r}
conditional_effects(RS.out, effects="cos_ZT:Group", 
                    conditions=data.frame(Chamber=levels(data.df$Chamber)))
conditional_effects(RS.out, effects="sin_ZT:Group", 
                    conditions=data.frame(Chamber=levels(data.df$Chamber)))
```
...
```{r}
pred.lFish <- posterior_epred(RS.out, newdata=data.df)
...
ggplot(prefChamber.df, aes(ZT, pred.mn)) + 
  geom_point(alpha=0.25) + 
  facet_wrap(~Group) + ylim(1, 5)

prefChamber.sum_epred %>%
  ggplot(aes(ZT)) + 
  geom_point(aes(y=pred.mn)) + 
  geom_linerange(aes(ymin=pred.q025, ymax=pred.q975)) +
  facet_wrap(~Group) +
  ylim(1, 5) +
  labs(x="ZT", y="Preferred chamber")
...
ggplot(out.df, aes(ZT, pred.mn, colour=Group, fill=Group, group=paste(Group, Tank))) + 
  ...
  facet_wrap(~Chamber) + 
  scale_colour_brewer(type="qual", palette=2) + scale_fill_brewer(type="qual", palette=2) + 
  ylim(0, NA) +
...
ggplot(out.df, aes(ZT, pred.lmn, colour=Group, fill=Group, group=paste(Group, Tank))) + 
  ...
  geom_line() +
  geom_ribbon(aes(ymin=pred.lq05, ymax=pred.lq95), alpha=0.25, colour=NA) +
  facet_wrap(~Chamber) +
  scale_colour_brewer(type="qual", palette=2) + scale_fill_brewer(type="qual", palette=2) + 
  theme(legend.position=c(0.8, 0.2)) +
